...
import { getOperationSearch } from "@/services/operation";
...
interface SelectProps {
  placeholder: string;
  selectedValue: { id: number; label: string } | null;
  onValueChange: (value: { id: number; label: string } | null) => void;
  className?: string;
  disabled?: boolean;
}
...
export default function SelectFilterOperations({
  placeholder,
  selectedValue,
  onValueChange,
  className,
  ...
  const fetchOperations = debounce(async (search) => {
    setIsLoading(true);
    try {
      const operations = await getOperationSearch(search);
      setOptions(
        operations.map((d) => ({
          value: d.id.toString(),
          label: `${d.id.toString()} - ${d.navio}`,
        })),
      );
    } catch (error) {
  ...
  return (
    <Select
      ...
      onChange={(newValue) =>
        newValue
          ? onValueChange({ id: Number(newValue.value), label: newValue.label })
          : onValueChange(null)
      }
      value={
        selectedValue
      ...
      classNames={{
        ...
        container: () => cn("w-full z-[2]"),
        control: ({ isDisabled, isFocused }) =>
          cn(
            "bg-transparent dark:bg-form-input border-stroke dark:border-form-strokedark hover:border-primary",
